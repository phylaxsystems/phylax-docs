...
The Phylax Credible Layer is a security framework that prevents smart contract exploits by enforcing user-defined security rules (assertions) at the sequencer level. It stops malicious transactions before they can affect your protocol.
...
5. **Economic Incentives**: Creates a market where sequencers earn fees for enforcing security rules

### What's the difference between assertions and traditional audits?
While audits provide one-time code reviews, assertions offer continuous protection against:
...
An assertion is a Solidity contract that defines invalid states your protocol should never reach. Common examples include:
...
the Credible Layer's approach to security:
...
Assertions run off-chain through the PhEVM (Phylax EVM) and can:
...
[Cheatcodes Reference Guide](/assertions-bookgit/cheatcodes).
...
### Why would sequencers or block builders implement the Credible Layer?
The Credible Layer creates value in multiple ways:
1. **Economic Incentives**: Assertion Enforcers earn fees through the Assertion Enforcement Market
2. **Network Value**: Increases transaction and network value by enabling additional security rules
3. **Market Creation**: Enables a new market for security services

### How does PCL work at the sequencer level?
The Credible Layer uses a modified Flashbots builder that:
...
3. Manages state progression during validation
4. Applies mitigations if needed
5. Ensures the L2 chain never stalls
...
Need more help? Join our [Telegram](https://t.me/phylax_credible_layer).
