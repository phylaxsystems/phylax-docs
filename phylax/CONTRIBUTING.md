...
Thanks for your interest in improving Phylax!
...
The [dev Telegram](telegram) is available for any concerns you may have that are not covered in this guide.

If you contribute to this project, your contributions will be made to the project.

### Code of Conduct

The Phylax project adheres to the [Rust Code of Conduct][rust-coc]. This code of conduct describes the _minimum_ behavior expected from all contributors.

Instances of violations of the Code of Coeduct can be reported by contacting the team
at [odysseas@phylax.watch](mailto:odysseas@phylax.watch).
...
1. **By opening an issue:** For example, if you believe that you have uncovered a bug
   in Phylax, creating a new issue in the issue tracker is the way to report it.
...
You can view the projects that we are currently working on at our public kanban on [Plane](https://plane.sh/phylax/0e315c15-d40b-4b21-83d9-5ee7f780c623?board=list).
...
As Phylax is still in heavy development, the documentation can be a bit scattered. The [Phylax Graph][phylax-graph] is our current best-effort attempt at keeping up-to-date information.
...
- The phylax version you are on (and that it is up to date)
- The platform you are on (macOS, an M1 Mac or Linux)
...
Pull requests are the way concrete changes are made to the code, documentation, and dependencies of Phylax.

Even tiny pull requests, like fixing wording, are greatly appreciated. Before making a large change, it is usually a
good idea to first open an issue describing the change to solicit feedback and guidance. This will increase the likelihood of the PR getting merged.
...
If the change being proposed alters code, it is either adding new functionality to Phylax, or fixing existing, broken functionality.

In both of these cases, the pull request should include one or more tests to ensure that Phylax does not regress in the future.
...
e.g. `cargo test mytest`) navigate to the directory of that file (e.g. `phylax/crates/phylax-core`) or use the `-p <package-name>` option to run specific tests of a crate from anywhere in the
workspace (`cargo test -p phylax-core mytest`).
...
**Any Phylax community member is welcome to review any pull request**.
...
1. Does this change make sense for Phylax?
2. Does this change make Phylax better, even if only incrementally?
...
can typically be fixed by the Phylax maintainers merging the pull request, but they can also be an opportunity for the
...
of the pull request. Yes, we may merge a particular change that makes Phylax better, but the individual might just not
want to have anything to do with Phylax ever again. The goal is not just having good code.

##### Abandoned or stale pull requests

If a pull request appears to be abandoned or stalled, it is polite to first check with the contributor to see if they intend to continue the work before checking if they would mind if you took it over (especially if it just has nits
left). When doing so, it is courteous to give the original contributor credit for the work they started, either by preserving their name and e-mail address in the commit log, or by using the `Author:` or `Co-authored-by:` metadata tag in the commits.
...
[telegram]: <https://t.me/+m1u_Qz3M33gyN2Y0>

[phylax-graph]: <https://graph.phylax.watch>
