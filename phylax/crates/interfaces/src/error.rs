...
/// The errorkind which implementors of the Activity trait can return.
///
/// The error variants which represent two possible states for when processing a message - a
/// recoverable result which only needs to be logged, or an unrecoverable error that should
/// cause the task to panic and the Phylax node to shutdown.
#[derive(thiserror::Error, Debug)]
pub enum PhylaxError {
    /// A critical error that indicates the task should panic and shutdown the Phylax node.
    #[error("Activity returned an unrecoverable error: {0:?}")]
    UnrecoverableError(#[source] Box<dyn Error + Send>),
    /// A recoverable error that indicates the task had a failure but should continue to process.
    #[error("Activity returned a recoverable error: {0:?}")]
    RecoverableError(#[source] Box<dyn Error + Send>),
}
...
pub enum EventBusError {
    /// A critical error that indicates some critical components have been dropped (all senders or
    /// receivers).
    #[error("The EventBus had a problem: {0:?}")]
    CriticalError(#[source] Box<dyn Error + Send>),
    /// A recoverable error that indicates the task had a failure but should continue to process.
    #[error("The receiver has lagged so far behind that it skipped a message: {0:?}")]
    SkippedMessageError(#[source] RecvError),
}
