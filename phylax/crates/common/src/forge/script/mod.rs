use self::{build::BuildOutput, runner::ScriptRunner, sequence::ScriptSequence};
use super::{build::BuildArgs, script::verify::VerifierArgs};
...
pub struct ScriptArgs {
    ...
    #[clap(
        long,
        requires = "sender",
        conflicts_with_all = &["private_key", "private_keys", "froms",  "aws"],
    )]
    pub unlocked: bool,
...
impl ScriptArgs {
    ...
    pub async fn show_traces(
        ...
        if result.success {
            info!("forge script executed succesfully");
        }
        ...
        if !result.success {
            ...
            eyre::bail!("{}", revert_msg);
        }
        Ok(())
    }

    pub fn ensure_not_revert(&self, result: &ScriptResult) -> Result<()> {
        if !result.success {
            let revert_msg = decode::decode_revert(&result.returned[..], None, None)
                .map(|err| format!("Revert Reason: '{err}'"))
                .unwrap_or_else(|_| "Script reverted without a revert string".to_string());
        ...
        for (data, to) in result.transactions.iter().flat_map(|txes| {
                ...
                bytecodes.iter().find(|(_, init_code, _)| *init_code == &data[offset..])
            {
                ...
                if deployment_size > max_size {
                    phylax_tracing::tracing::error!("`{name}` is above the contract size limit ({deployment_size} > {max_size})");
                }
            }
        }
...
pub struct ScriptArtifact {
    pub receipts: HashMap<Chain, Vec<TransactionReceipt>>,
}

impl From<Vec<ScriptSequence>> for ScriptArtifact {
    fn from(value: Vec<ScriptSequence>) -> Self {
        let mut receipts: HashMap<Chain, Vec<TransactionReceipt>> = HashMap::new();
        for sequence in value {
            let chain = match sequence.chain.try_into() {
                Ok(chain) => chain,
                Err(_) => continue, // Skip if the chain cannot be converted
            };
            if let Some(receipts_for_chain) = receipts.get_mut(&chain) {
                receipts_for_chain.extend(sequence.receipts.iter().cloned());
            } else {
                receipts.insert(chain, sequence.receipts.clone());
            }
        }
        Self { receipts }
    }
...
impl ScriptConfig {
    ...
    fn check_multi_chain_constraints(&self, libraries: &Libraries) -> Result<()> {
        if (self.has_multiple_rpcs() || (self.missing_rpc && !self.total_rpcs.is_empty())) &&
            !libraries.libs.is_empty()
        {
            eyre::bail!("Multi chain deployment does not support library linking at the moment.")
        }
        Ok(())
    }
...
For more information, please see https://eips.ethereum.org/EIPS/eip-3855"#,
            ...
            phylax_tracing::tracing::error!(msg);
        }
        Ok(())
    }
}
