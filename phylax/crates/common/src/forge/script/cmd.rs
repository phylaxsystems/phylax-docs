use super::{multi::MultiChainSequence, sequence::ScriptSequence, *};
use crate::forge::script::verify_bundle::VerifyBundle;
...
impl ScriptArgs {
    ...
    pub async fn run_script(
        ...
        trace!(target: "forge::script", "executing script command");
        ...
        if self.resume {
            ...
            .await?;
            let artifact = ScriptArtifact::default();
            return Ok(artifact);
        }
        ...
        let sequence = self
            .handle_broadcastable_transactions(
                result.clone(),
                libraries,
                &decoder,
                script_config,
    ...
    async fn rerun_with_new_deployer(
        ...
        let result = self
            .execute(script_config, contract, new_sender, &predeploy_libraries, context_map)
            .await?;
