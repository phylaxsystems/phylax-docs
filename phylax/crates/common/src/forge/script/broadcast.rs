use super::{
    ...
    sequence::ScriptSequence, transaction::TransactionWithMetadata, verify_bundle::VerifyBundle, *,
};
use ethers::{
...
use phylax_tracing::tracing::{debug, trace, warn};
...
impl ScriptArgs {
    ...
    pub async fn send_transactions(
        ...
        if already_broadcasted < deployment_sequence.transactions.len() {
            ...
            for (batch_number, batch) in sequence.chunks(batch_size).map(|f| f.to_vec()).enumerate()
            {
                ...
                debug!(
                    tx_number = batch_number * batch_size,
                    tx_total = batch_number * batch_size + min(batch_size, batch.len()) - 1,
                    "Sendin Transactions"
                );
        ...
        let (_total_gas, total_gas_price, total_paid) = deployment_sequence.receipts.iter().fold(
        ...
        info!(total_paid = paid, average = avg_gas_price, "Action gas");
        Ok(())
    }
    ...
    async fn send_transaction(
        ...
        match kind {
            SendTransactionKind::Unlocked(addr) => {
                phylax_tracing::tracing::debug!(
                    "sending transaction from unlocked account {:?}: {:?}",
                    addr,
                    tx
                );
    ...
    pub async fn handle_broadcastable_transactions(
        ...
        if let Some(txs) = result.transactions.take() {
            ...
            if !script_config.missing_rpc {
                trace!(target: "forge::script", "creating deployments");
                ...
                if script_config.has_multiple_rpcs() {
                    trace!(target: "forge::script", "broadcasting multi chain deployment");
                ...
                if !self.broadcast {
                    info!(target: "forge::script", "Simulation Complete");
                }
            } else {
                warn!("RPC url missing; can't simulate transactions without an RPC url");
            }
        }
        Ok(deployments)
    }
    ...
    async fn fills_transactions_with_gas(
        ...
        let gas_filled_txs = if self.skip_simulation {
            debug!("Skipping on-chain simulation");
            txs.into_iter()
                .map(|btx| {
        ...
        while let Some(mut tx) = txes_iter.next() {
            ...
            if !self.skip_simulation {
                ...
                if has_different_gas_calc(provider_info.chain) {
                    trace!(target: "forge::script", "estimating with different gas calculation");
                    ...
                    if let Err(err) = self.estimate_gas(typed_tx, &provider_info.provider).await {
                        trace!(target: "forge::script", "gas estimation failed: {err}");
        ...
        if !self.skip_simulation {
            ...
            for (rpc, total_gas) in total_gas_per_rpc {
                ...
                debug!(
                    ...
                    estimated_gas_gwei = %format_units(per_gas.to_ethers(), 9)
    ...
    async fn broadcast(
        ...
        phylax_tracing::tracing::debug!("Broadcasting Transaction: {:?}", legacy_or_1559);
