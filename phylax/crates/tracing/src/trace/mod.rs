...
///  Tracing subscriber builder for application logging.
///
...
#[derive(Debug, Clone)]
pub struct PhylaxTracingBuilder {
    stdout: LayerInfo,
    journald: Option<String>,
    file: Option<(LayerInfo, FileInfo)>,
}

impl PhylaxTracingBuilder {
...
impl Default for PhylaxTracingBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
pub trait Tracer {
    ...
    ///  # Returns
    ///  An `Result` which is `Ok` with an optional `WorkerGuard` if a file layer is used,
    ///  or an `Err(PhylaxTracingError)` in case of an error during initialization.
    fn init(self) -> Result<Option<WorkerGuard>, PhylaxTracingError>;
}

impl Tracer for PhylaxTracingBuilder {
...
pub fn init_test_tracing() {
    let _ = test::TestTracer::default().init();
}
