...
If any of those states for some reason are reached the Phylax Monitor will notify the user and the user can act accordingly.
...
Since the default `bool` value of Solidity is `false` the alert will trigger a notification if the condition is `true`.
This might seem a bit counter-intuitive at first but it makes sense, since we don't want any false negatives.

Below is a very simple example of an alert function that checks if the admin of the contract remains the same:
...
This will return `false` as long as the admin of the contract is the same as the `PREDEFINED_DESIRED_ADMIN`.
If the admin is changed the alert will return `true` and a notification can be triggered.
...
We then check that the `totalAssets()` of the vault is not `0`. It is unlikely that the vault should reach a balance of `0`, so we want to be notified if that should happen. 
It could mean that someone has drained the vault or that the vault has been exploited.
...
contract VaultAlert is Alert, Notification {
    ...
    function alertVaultDrain() public returns (bool, Notification.NotificationMessage) {
        ...
        if (vault.totalAssets() != 0) {
            return true, info("All good! The balance of the vault is higher than 0");
        } else {
...
You can find many examples of alerts for popular protocols in the [phylax-std GitHub](https://github.com/phylax-labs/phylax-std/tree/main/examples/alerts).
