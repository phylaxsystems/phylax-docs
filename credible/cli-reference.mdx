...
| [`pcl deploy`](#deploy) | Create or resolve a project, create a release, and activate it on-chain |
...
| `--keystore-password-file PATH` | File containing the Foundry keystore password. Can also be set with `PCL_KEYSTORE_PASSWORD_FILE` |
| `--rpc-url RPC_URL` | RPC endpoint used to broadcast transactions. Can also be set with `PCL_RPC_URL` |
| `--confirmations CONFIRMATIONS` | Confirmations to wait for after broadcasting. Values below the platform requirement are rejected |
| `--tx-timeout-secs SECONDS` | Transaction confirmation timeout. Default: `300` |
| `--project-name PROJECT_NAME` | Project name used when `credible.toml` has no `project_id`; overrides `project_name` in the file |
| `--chain-id CHAIN_ID` | Chain ID used when creating a project. Existing project chains are read from the platform |
| `--skip-protocol-manager` | Skip protocol-manager verification or configuration |
| `--check-timeout-secs SECONDS` | Timeout for deployment-blocking release checks. Default: `600` |
| `-j, --json` | Emit strict JSON output |
| `--llms` | Print the CLI-native LLM usage guide and exit |
| `-h, --help` | Print help |
...
```bash
pcl deploy --dry-run
pcl deploy --account protocol-manager --rpc-url https://example-rpc.invalid
pcl deploy --project-name my-protocol --chain-id 84532 --account protocol-manager --rpc-url https://example-rpc.invalid --yes --json
```
