...
1. Navigate to your project in the [Phylax platform](https://app.phylax.systems)
...
1. In the Credible Layer Slack settings, click **Connect to Slack**
...
<Tip>
Click **Connect to PagerDuty** in the Credible Layer settings to open PagerDuty's integration guide.
</Tip>
</Step>
...
<Step title="Test the Integration">
1. After saving, click **Send Test Invalidation**
...
You receive one notification per invalidation group, not per transaction. An invalidation group contains transactions that violate the same assertion for the same assertion adopter, chain, and environment within a 60-minute window.

If an attacker crafts multiple transactions that all violate the same assertion, they are grouped together within that window. After 60 minutes without violations, a new invalidation creates a new group.
...
- Project admins and editors can configure integrations
