...
1. **Off-chain Execution**: Rules that would be too expensive or impossible to express on-chain due to gas costs and limits can be implemented easily as assertions as they are run off-chain. Our current benchmarks show around 1000x efficiency gains and we are able to add whatever precompiles we want.
...
6. **Aligned Economic Incentives**: Sequencers earn fees for protecting protocols rather than extracting value from the ecosystem
...
- If the **`require`** fails, the transaction is invalid and will be dropped by the network
