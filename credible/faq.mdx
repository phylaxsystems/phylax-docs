...
The Credible Layer simply lets dApps define assertions that can prevent certain states regardless of the contract code. You can think of it as an override: if the contract were to execute a hack, the assertion will signal to the sequencer to prevent it, as it has been defined as a hack by the dApp. These assertions should be thought of as an addition to the dApps' core contracts. It would be censorship if a 3rd party (anyone other than the dApp) could write assertions for a dApp's contracts. This is not how the Credible Layer works, and hence this is not censorship.
