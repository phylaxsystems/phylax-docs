...
flowchart TD
    ...
    MP -->|"2"| AV["Assertion Validation"]
    
    AV -->|"3a"| REF["References assertions"]
    AV -->|"3b"| FETCH["Fetches assertion bytecode"]
    REF -->|"4"| PhEVM["PhEVM executes assertions"]
    FETCH -->|"4"| PhEVM
    
    PhEVM -->|"5a"| INVALID["Transaction invalid"]
    PhEVM -->|"5b"| VALID["Transaction valid"]
    
    INVALID -->|"6a"| REJECT["Reject transaction"]
    VALID -->|"6b"| INCLUDE["Include in block"]
    
    INCLUDE -->|"7"| FINAL["Block Finalization"]
    
    classDef step fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef fetch fill:#fff8e1,stroke:#f57c00,stroke-width:2px
    ...
    class TX,MP,AV,FINAL step
    class REF,FETCH fetch
    class PhEVM process
...
Want to try the Phylax Credible Layer locally? Explore these resources:
