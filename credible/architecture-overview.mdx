...
**Development Flow**: Developers use [`pcl`](/credible/store-submit-assertions) to develop and test assertions. `pcl` stores assertion code in [Assertion DA](/credible/assertion-da), then submits to the [Phylax platform](https://app.phylax.systems). The platform deploys assertions to the on-chain [Credible Layer Contracts](/credible/credible-layer-contracts) so they can be enforced by the network.
...
1. **Assertion Deployment**: Developers write and submit assertions through `pcl` and the platform, which are stored in Assertion DA and deployed on-chain
...
---
flowchart LR
    ENFORCER["Assertion Enforcer"] --> DAPP["Phylax Platform"]
