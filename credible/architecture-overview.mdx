...
**Development Flow**: Developers use [`pcl`](/credible/apply-assertions) to develop and test assertions. Running `pcl apply` compiles assertion code and creates a release on the [Phylax platform](https://app.phylax.systems), which handles storing it in [Assertion DA](/credible/assertion-da). An authorized protocol manager then reviews the release and signs its deployment transaction to the on-chain [Credible Layer Contracts](/credible/credible-layer-contracts).
...
1. **Assertion Deployment**: Developers write assertions and create a release with `pcl apply`. The platform stores assertion bytecode in Assertion DA, and an authorized protocol manager signs the on-chain deployment
