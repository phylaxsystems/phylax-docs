...
**Development Flow**: Developers use the `pcl` to develop and test assertions. The `pcl` stores assertion bytecode directly in [Assertion DA](/credible/assertion-da), then submits to the Credible dApp with the assertion ID. The dApp registers these assertions in the on-chain Credible Layer contracts.
