...
- **Network-wide enforcement by default**: assertions only affect networks and environments where the enforcer is integrated and the assertion is active
...
- **Enforcer unavailable**: the network continues normal operation without enforcement until the
  enforcer is restored. This is a fail-open model: a Credible Layer outage should not halt the chain, but protected protocols temporarily lose enforcement.
...
- **Incorrect assertion logic**: a false positive can block honest production transactions, while an incomplete assertion can miss the intended failure mode

## Operational Controls

Teams reduce these risks by validating assertions locally, backtesting against historical transactions, running assertions in staging, and promoting to production only after sufficient review. See [How to Plan Assertion Testing](/credible/testing-strategy) for the recommended workflow.
