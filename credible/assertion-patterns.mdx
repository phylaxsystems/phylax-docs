...
<Note>
For information about using triggers to optimize when assertions run, see the [Triggers](/credible/triggers) documentation.
</Note>
...
contract OwnerChangeAssertion is Assertion {
    ...
    function assertOwnerChange() external {
        IOwnable target = IOwnable(ph.getAssertionAdopter());
