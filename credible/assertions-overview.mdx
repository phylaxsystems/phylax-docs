...
- The [PhEVM](/credible/glossary#phevm-phylax-evm) simulates the transaction and creates pre/post state snapshots
...
Regular Solidity only sees the current state during execution. Assertions can compare values before and after a transaction using `ph.forkPreTx()` and `ph.forkPostTx()`. You can also inspect state at any point during execution using `ph.forkPreCall()` and `ph.forkPostCall()` to check individual nested calls within the transaction.
