---
title: 'Write Your First Assertion'
description: 'Build a complete assertion that protects ownership changes'
---
...
**Prerequisites**: Familiarity with Solidity, the [Assertions Overview](/credible/assertions-overview), and [`pcl` installed](/credible/credible-install).
...
For manual setup, see the [Quickstart Guide](/credible/pcl-quickstart).
...
The `Assertion` base class provides [cheatcodes](/credible/glossary#cheatcodes) via the `ph` namespace.
...
See [Triggers](/credible/triggers) for more trigger types and optimization.
...
- [`cl.assertion()`](/credible/testing-assertions#credible-layer-testing-interface): Registers the assertion to run on the next transaction
...
For more details on testing assertions, see [Testing Assertions](/credible/testing-assertions).
