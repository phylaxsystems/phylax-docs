...
description: 'Configure and deploy assertions to the platform using `pcl apply`'
---

This guide walks you through configuring your assertions in `credible.toml` and deploying them to the platform with `pcl apply`.
...
**What you'll do**: Authenticate, configure `credible.toml`, and deploy your assertions.
...
This opens a browser window where you can authenticate with your wallet or email. Once approved, `pcl` automatically detects the authentication.
...
Deploy your assertions to the platform:
...
<Note>
Applying through `pcl` does not make the assertion active by itself. The assertion becomes active only after you deploy it through the platform and the configured timelock in the on-chain State Oracle contracts expires. Removals follow the same State Oracle timelock flow.
</Note>
