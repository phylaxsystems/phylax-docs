...
| OutOfGas error | Assertion exceeds the default 3,000,000 gas limit | Optimize loops, add early returns, fail fast |
...
This error means your assertion never ran on the target transaction. This can occur in local tests, staging, production, or a platform release backtest.

**Common Causes:**

**In staging, production, or a platform release backtest:**
...
**Production Behavior:**
- If an assertion reverts, the Enforcer returns an invalid result and the network applies its inclusion policy
...
Assertion execution has a default **3,000,000 gas limit**. Node operators can configure this limit. If an assertion exceeds the active limit, it reverts with `OutOfGas` and the Enforcer returns an invalid result even if there's no actual violation. The network then applies its inclusion policy.

**Symptoms:**

- `OutOfGas` error when running tests
- "Unknown Revert Reason" with gas cost near the active assertion limit in a platform release backtesting result
...
## Release backtesting findings

The platform runs backtesting automatically when you create a release. An **Issue found** result means at least one historical transaction from the last 20,000 blocks would have been invalidated by the release.

Open **Backtesting** in the release and review the matched transaction hash, assertion ID, adopter, incident payload, previous transactions, and block environment. If the finding is benign, tune the assertion and create a new release. If the evidence does not explain the finding, do not deploy the release. See [How to Review Backtesting Results](./backtesting) for the complete workflow.
...
- Gas consumption hitting the active assertion limit
