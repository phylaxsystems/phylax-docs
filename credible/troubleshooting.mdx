...
Assertion execution has a default **3,000,000 gas limit**. Node operators can configure this limit. If an assertion exceeds the active limit, it reverts with `OutOfGas`—causing the transaction to be dropped even if there's no actual violation.
