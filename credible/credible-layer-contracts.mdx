...
When adding assertions, a proof of availability must be provided. The current implementation (`DAVerifierECDSA`) requires a signature from the [Assertion DA](/credible/assertion-da) prover, which is generated when deploying assertions via [`pcl apply`](/credible/cli-reference#apply).
