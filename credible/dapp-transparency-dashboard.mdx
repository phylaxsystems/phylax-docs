...
Detailed invalidation data is intentionally private by default. A rejected transaction can reveal information about an attempted exploit or an assertion's exact failure path, so full traces and payloads are scoped to project managers and approved collaborators.
