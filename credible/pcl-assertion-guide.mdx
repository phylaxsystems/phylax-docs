...
Assertion functions are limited to a maximum of 300,000 gas. If an assertion function exceeds this gas limit, the transaction will be invalidated and dropped.
...
See the [Assertions overview](/credible/assertions-overview#constraints) for more details on gas limits and optimization strategies.
...
- In this example we entirely leave out the constructor, since we use the `ph.getAssertionAdopter()` cheatcode to get the address of the contract we want to protect.
...
- The Credible Layer supports different trigger types. See the "Triggers" section in the [cheatcodes documentation](/credible/cheatcodes-reference) for more information.
...
- The main assertion logic uses cheatcodes:
  - `ph.forkPreTx()`: Creates a snapshot of the blockchain state before the transaction.
  - `ph.forkPostTx()`: Creates a snapshot of the blockchain state after the transaction.
...
- The `ph` namespace gives access to the Credible Layer cheatcodes, which provide utility functions for assertions. For more information see the [cheatcodes documentation](/credible/cheatcodes-reference).
...
2. **State Management**: Use `forkPreTx()` and `forkPostTx()` to compare values before and after transactions.
3. **State Management**: Use `forkPreCall()` and `forkPostCall()` to compare values before and after specific calls.
4. **Gas Optimization**: Use triggers to make sure that assertions are only called when they are needed in order to save gas and resources.
5. **Return Early**: When writing more complex assertions checking multiple function calls in the callstack, it is recommended to check basic invariants first before starting to iterate over the callstack.
6. **Debugging**: Use `console.log()` in assertion functions to print debug information to the console during testing with `pcl test`.
...
- `OwnableAssertion`: Imports the assertion we created that we want to test
...
```
Key components:
1. `cl.assertion()`: Sets up the assertion to be run on the next transaction
   - `adopter`: Address of contract to protect
   - `createData`: Bytecode of the assertion contract
   - `fnSelector`: Selector of the assertion function to run
2. `vm.prank()`: Simulates a call from the owner's address
3. `vm.expectRevert()`: Explicitly expects the assertion to revert with "Ownership has changed"
4. `assertionAdopter.transferOwnership(newOwner)`: Simulates a transaction that changes ownership
5. `assertEq(assertionAdopter.owner(), initialOwner)`: Checks that the owner didn't change
...
- `transferOwnership` will not revert if the assertion passes so we don't need to use `vm.expectRevert()`
...
1. **Assertion Registration**: Use `cl.assertion()` to register assertions that will be run on the next transaction
2. **State Manipulation**: Use Forge's cheatcodes (`vm.prank`, `vm.deal`) to set up test scenarios
3. **Verification**: Use `vm.expectRevert()` to verify that the assertion reverts when expected
...
- Learn about the [Cheatcodes](/credible/cheatcodes-reference)
- Follow the [Quickstart](/credible/pcl-quickstart) to learn how to deploy your assertions
- If you encounter issues, check the [Troubleshooting Guide](/credible/troubleshooting)
- For a comprehensive list of terms and concepts, see the [Glossary](/credible/glossary).
