...
<Note>
The PCL framework supports different trigger types. `registerCallTrigger` is used to run assertions after any transaction interacting with the protected contract. Other triggers can be implemented depending on your needs.
...
- Explore the assertion examples in the [Assertions Book](/assertions-book/assertions-book-introduction)
...
- **Assertion Group**: A collection of assertions in the same contract that are related to the same protocol contract
- **Assertion Function**: A function in an assertion contract that performs a specific check
- **Assertion Test**: A test that verifies the correctness of an assertion
- **PCL (Phylax Credible Layer)**: The framework that runs assertions against transactions to prevent hacks
- **Cheatcodes**: The set of functions that are available to the assertion contract to interact with the blockchain
- **Trigger**: A mechanism that determines when an assertion should be executed
