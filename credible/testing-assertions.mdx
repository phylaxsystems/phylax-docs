...
If an assertion exceeds this limit, it reverts with `OutOfGas`. In production, this causes the transaction to be dropped - even if there's no actual violation. This makes gas limit issues critical to catch during testing.
...
- Learn more about [Writing Assertions](/credible/write-first-assertion)
