...
## Assertion Admission and Resource Limits

The enforcer should not treat published assertions as an unbounded execution surface. A network-integrated deployment should only execute assertions that are active for the relevant environment and admitted through the configured registry and review process.
...
These controls protect the network from assertion griefing: broad, expensive, mutable, or useless assertions consuming block-building resources. They also help protocol teams understand which assertions are safe to promote from staging to production.
