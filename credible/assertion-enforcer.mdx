...
Ingress filtering can also reduce repeated invalid submissions. When a transaction invalidates, the integration can record short-lived fingerprints for exact replays or repeated attempts against the same assertion and target surface. Broad fingerprints should use conservative TTLs and thresholds.
...
- **Transparent outcomes**: Invalidations are visible in the platform and Phylax Explorer
