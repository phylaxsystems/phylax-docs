...
description: 'Tutorial: write, submit, and deploy your first assertion with `pcl`'
---
...
<Note>
If you already know the overall flow and only need one task, use the dedicated guides for [installation](./credible-install), [storing and submitting assertions](./store-submit-assertions), or [deployment in the dApp](./deploy-assertions-dapp).
</Note>
...
<Note>
If authentication fails, first confirm that your wallet is on the correct network and that `pcl` is using the expected auth URL. Then use the [Troubleshooting guide](./troubleshooting) for the common failure modes.
</Note>
...
For a more detailed overview of how to use the dApp and manage projects, see the [Deploy Assertions guide](./deploy-assertions-dapp).
...
To finish the tutorial, go to the dApp and deploy the assertion. Go back to the URL that you opened with the `pcl auth login` command and navigate to the project that the assertion was added to.
...
For a more detailed overview of how to use the dApp, see the [Deploy Assertions guide](./deploy-assertions-dapp).
...
For more detailed information about `pcl` and its commands, see [Store and Submit Assertions](./store-submit-assertions) and [pcl Reference](./cli-reference).
