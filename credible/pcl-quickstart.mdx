...
Once you have deployed your contract, create a project in the platform if you do not have one already. Navigate to [app.phylax.systems](https://app.phylax.systems), create the project, then open **Contracts** and add the address you deployed in the previous step.
...
Open **Releases**, select the release marked **Needs Review**, and review its changes and checks. Its **Staging** or **Production** environment comes from `credible.toml`. Click the corresponding deployment button and sign the transaction.
