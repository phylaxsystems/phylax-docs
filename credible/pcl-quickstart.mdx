...
7. Activate your assertion
8. Verify that the assertion is working
...
- Protocol smart contracts deployed to write assertions for OR access to a testnet that you can deploy smart contracts to

## 0. Lazy Mode

We've created an example project with the content of this guide that you can use to follow along and use as a starting point for your own projects.
...
You can jump directly to the [Running Tests](#4-running-tests) section if you have the example project cloned and `pcl` installed.
...
```bash
pcl store OwnableAssertion 0xADDRESS_OF_OWNABLE_CONTRACT
```
...
If your assertion contract has a constructor argument, you need to provide it when storing the assertion.
...
Next, let's attempt to transfer ownership to a new address. This transaction should trigger our assertion and revert:
...
The transaction should timeout after about 2 minutes which means that the assertion reverted the transaction:
