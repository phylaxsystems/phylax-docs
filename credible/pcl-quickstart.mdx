...
The transaction should fail silently as the assertion prevents the ownership change. You might not see an explicit error message, but the important thing is that the transaction doesn't succeed - this is the expected behavior since our assertion is designed to prevent ownership changes.
