...
description: 'Reference for the experimental flow-rate context returned by cumulative ERC20 flow triggers'
---
...
<Warning>
Both getters are experimental. An assertion that calls either getter must register `AssertionSpec.Experimental`; Legacy and Reshiram assertions cannot call them.
</Warning>
...
1. The assertion registers `watchCumulativeOutflow(...)`, `watchCumulativeInflow(...)`, or both in `triggers()`.
2. The executor records the assertion adopter's balance changes for the watched ERC20 token. All recorded changes in the same block are accumulated into one bucket.
3. At evaluation, the executor keeps buckets whose timestamps are inside the configured rolling window. The token balance immediately before the earliest retained bucket becomes `tvlSnapshot`.
4. The cumulative trigger calculates net directional flow across the window and invokes the assertion only when that amount is nonzero and strictly greater than `thresholdBps` of `tvlSnapshot`.
5. Inside the invoked assertion, the matching rate getter returns statistics calculated from those same retained buckets.

If a rate limit must be checked before the intended cumulative limit is reached, register the watcher with a lower dispatch threshold and enforce both limits inside the assertion, as shown in the [combined example](#combined-inflow-and-outflow-example).
