...
This assertion watches both flow directions for one ERC20 token. A low cumulative threshold dispatches the corresponding assertion function early. That function then rejects the transaction when either the configured cumulative limit or peak-rate limit is exceeded.
