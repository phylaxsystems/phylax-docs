...
## The cumulative limit remains the foundation

Rate-of-change enriches a cumulative breaker; it does not replace one. The rate getters are only available after a matching cumulative trigger fires, and a rate-only policy can miss a slow drain that stays below its velocity threshold.

Use the cumulative amount as the primary risk boundary. If the assertion permits a low-rate cumulative breach, handle the remaining cumulative risk through another limit, an alert, or an explicit operational policy. This keeps the rate signal from silently turning gradual loss into acceptable loss.

Rate values use integer division, so a result below `1 bps/s` is rounded down to zero. A nonzero flow against a zero TVL snapshot is treated as an unbounded rate.
