...
Check [Modified Key Discovery](/credible/use-case-mappings/modified-keys) for the V2 helpers that enumerate mapping entries changed by the current transaction. These helpers do not enumerate every position that has ever existed.
