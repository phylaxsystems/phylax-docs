...
- `getFunctionCallInputs()`: Gets the inputs to function calls for tracking position changes
...
For complete testing examples, refer to the [Phylax Assertions Repository](https://github.com/phylax/assertions).
...
4. **Error Messages**: Provide detailed error messages that include the expected and actual values to help with debugging when the assertion fails.

5. **Combining with Other Assertions**: This assertion works well with:
   - Balance consistency assertions
   - Supply cap assertions
   - Interest rate assertions
...
6. **Debugging Tips**:
   ...
   - Check for patterns in failures that might indicate specific vulnerabilities

## Related Assertions

- [Balance Consistency](ass07-lending-health-factor): Ensures user balances are consistent across different views
- [Supply Cap](ass16-liquidation-health-factor): Verifies that the total supply doesn't exceed protocol limits
